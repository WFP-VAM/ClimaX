...
from climax.utils.verif import plot_train
from climax.utils.metrics import (
    lat_weighted_mse,
    lat_weighted_mse_val,
    lat_weighted_rmse,
    lat_weighted_mean_bias,
    loss_function_training,
    pearson,
    mean_bias, 
    spearman,
)
...
class ClimateDownscalingModule(LightningModule):
    def __init__(
        ...
        self.net = net
        self.accumulate_grad_batches = 2  # Accumulate gradients over 4 mini-batches
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        # plot_train(x, y,batch_idx)
        out_h, out_w = y.shape[-2], y.shape[-1]
        x = torch.nn.functional.interpolate(x, (out_h, out_w), mode="bilinear")
        

        loss_dict, _ = self.net.forward(x, y, lead_times, variables, out_variables, [loss_function_training], lat=self.lat)
        ...
        #     self.log("train/loss_accumulated", loss)  # Log the accumulated loss
        #     return loss
        # else:
        #     return loss
        
        
        with open("tests/loss_log.txt", "a") as log_file:
            log_file.write(f"Batch {batch_idx}, Loss: {loss:.4f}\n")
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        ...
        all_loss_dicts = self.net.evaluate(
            ...
            metrics=[lat_weighted_mse_val, lat_weighted_rmse, pearson, mean_bias, spearman],
            lat=self.lat,
            clim=None,
